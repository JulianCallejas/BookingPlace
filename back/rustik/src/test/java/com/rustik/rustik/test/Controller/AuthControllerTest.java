...
public class AuthControllerTest {
    ...
    public void testValidateToken_Success() throws Exception {
        ...
        CustomUserDetails userDetails = new CustomUserDetails(new User("test@example.com", "Test User", "123456789"));
        when(tokenService.subjectIsAdmin(anyString())).thenReturn(false);

        ResultActions result = mockMvc.perform(get("/api/v1/auth/validate-token")
                .header("Authorization", "Bearer " + token)
                .principal(() -> userDetails));

        result.andExpect(status().isOk())
                .andExpect(jsonPath("$.email").value(userDetails.getUser().getEmail()));
    }
